  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3.5 py-2 text-sm font-medium text-gray-900 hover:bg-gray-50 cursor-pointer"
      >
        {selected}
...
export default function UserTable() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["users-table-list", filter],
    queryFn: async () => {
      const res = await api.get(`/users/table-list`, {
        params: { type: filter },
      });
      return res?.data ?? res;
    },
            ...
            {isLoading ? (
              ...
              rows.map((row, index) => (
                <tr key={row?.id || index}>
                  ...
                  <td className="whitespace-nowrap border-b border-gray-50 px-3 py-4">
                    <span
                      className="cursor-pointer font-semibold text-indigo-700 hover:underline"
